...
from school.models import FeeRecord, StudentExtra
from reportlab.platypus import Table, TableStyle
import os

def remove_all_fee_records():
    FeeRecord.objects.all().delete()
#remove_all_fee_records()
...
def student_signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            # Set the fee based on the selected class
            class_fee_mapping = {
                'one': 1500,
                'two': 1700,
                'three': 1900,
            ...
            selected_class = form2.cleaned_data['cl']
            fee = class_fee_mapping.get(selected_class, 0)
            f2.fee = fee
...
def afterlogin_view(request):
    ...
    # Default return statement in case none of the conditions above are met
    return HttpResponse("Invalid user type or not logged in.")
...
def admin_view_attendance_view(request,cl):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            attendance_query = Q(cl=cl)
            
            if date:
                attendance_query &= Q(date=date)
                
            if month:
                attendance_query &= Q(date__month=month)
                
            if year:
                attendance_query &= Q(date__year=year)
                
            if roll:
                attendance_query &= Q(roll=roll)
                
            attendancedata = models.Attendance.objects.filter(attendance_query)
...
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_add_fee_view(request, cl):
    ...
    if request.method == 'POST':
        year = int(request.POST.get('year', 2023))  # Get the selected year from the form

        for student in students:
            is_paid = request.POST.get('paid_' + str(student.id), 'no') == 'yes'
            month = request.POST.get('month_' + str(student.id))  # Get the selected month value

            if month:  # Check if the month field is provided
                fee_amount = class_fee_mapping.get(cl, 0) if is_paid else 0

                if not is_paid:
                    # Skip saving the fee record if the status is not "Paid"
                    continue

                # Check if a paid entry already exists for the selected month and year
                existing_record = models.FeeRecord.objects.filter(student=student, month=month, year=year, status='Paid').first()
                if existing_record:
                    error_message = f"Fee record for {student.get_name()} and {month} already exists as paid."
                    error_messages.append(error_message)
                    messages.error(request, error_message)
                    continue

                try:
                    fee_model = models.FeeRecord(
                        student=student,
                        month=month,
                        year=year,
                        status='Paid',
                    ...
                    # Handle the case when the same entry already exists
                    error_message = f"Fee record for {student.get_name()} and {month} already exists."
                    error_messages.append(error_message)
                    messages.error(request, error_message)
    ...
    return render(request, 'school/admin_add_fee.html', {'students': students, 'class_fee_mapping': class_fee_mapping})
...
def admin_view_fee_view(request, cl):
    students = models.StudentExtra.objects.filter(cl=cl)
    fee_records = models.FeeRecord.objects.filter(student__in=students, status='Paid')
    return render(request, 'school/admin_view_fee.html', {'feedetails': students, 'fee_records': fee_records, 'cl': cl})
...
def teacher_dashboard_view(request):
    teacherdata = models.TeacherExtra.objects.filter(user=request.user).first()
    notice = models.Notice.objects.all()

    # Retrieve the total number of students for the teacher's class
    total_students = models.StudentExtra.objects.filter(cl=teacherdata.class_incharge).count()

    mydict = {
        'salary': teacherdata.salary,
        'mobile': teacherdata.mobile,
        'date': teacherdata.joindate,
        'class': teacherdata.class_incharge,
        'notice': notice,
...
def teacher_attendance_view(request):
    teacher = models.TeacherExtra.objects.filter(status=True, user_id=request.user.id).first()
    if not teacher:
        # Handle the case when the teacher does not exist or is not in charge of any class
        return HttpResponse('Teacher does not exist or is not in charge of any class')

    return render(request, 'school/teacher_attendance.html', {'teacher': teacher})
...
def student_attendance_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Get the attendance data based on the user input
            if date:
                attendancedata = models.Attendance.objects.filter(date=date, cl=studentdata[0].cl, roll=studentdata[0].roll)
            elif month and year:
                month_number = int(month)  # Convert month to integer
                attendancedata = models.Attendance.objects.filter(date__month=month_number, date__year=year, cl=studentdata[0].cl, roll=studentdata[0].roll)
            else:
                # If no specific date, month, or year is provided, get all attendance data
                attendancedata = models.Attendance.objects.filter(cl=studentdata[0].cl, roll=studentdata[0].roll)
...
def student_fee_view(request):
    ...
    feedetails = student.feerecord_set.all()
    return render(request, 'school/student_view_fee.html', {'feedetails': feedetails})

@login_required(login_url='studentlogin')
@user_passes_test(is_student)
def download_fee_challan(request):
    if request.method == 'POST':
        student = request.user.studentextra
        ...
        response['Content-Disposition'] = f'attachment; filename=fee_challan_{name}_{month}_{year}.pdf'
        ...
        # Add school logo
        logo_path = 'D:/Study/SchoolManagement/static/images/logo_sc.png'
        p.drawImage(logo_path, 250, 650, width=100, height=100, preserveAspectRatio=True)

        # Add heading
        p.setFont("Helvetica-Bold", 18)
        p.drawCentredString(300, 600, "Fee Challan")

        # Draw the table
        data = [
            ["Student Name", "Class", "Roll Number", "Fee Amount", "Month", "Year"],
            [name, cl, roll, fee, month, year],
        ]
        table = Table(data, colWidths=[100, 100, 100, 100, 100, 100], hAlign="LEFT")
        ...
        table.drawOn(p, 5, 500)

        # Draw the "Stamp" box
        p.rect(450, 50, 100, 100, fill=0)
        p.setFont("Helvetica-Bold", 14)
        p.drawString(460, 70, "Stamp")
    ...
    return render(request, 'school/fee_challan.html')
